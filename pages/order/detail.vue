    ...
    <view class="page-body">
      <view class="page-section page-section-gap">
        ...
        >
          ...
          <cover-image
            class="midImage"
            src="../../static/drawable-xhdpi/dingdan_xiangqing_icon1.png"
          >
          </cover-image>
          ...
          <cover-view class="onCenter"></cover-view>
          <cover-view
            :style="[{ top: item1.leftTop + 'px' }]"
            class="float-left"
            v-for="(item1, index1) in leftList"
            :key="index1"
            >{{ item1.name }}</cover-view
          >
          <cover-view class="cutOff"></cover-view>
          <!-- 华丽的分割线 -->
          <block v-for="(item2, index2) in rightList" :key="index2">
            <cover-view
              :style="[{ top: item2.rightTop + 'px' }]"
              class="float-right"
              >{{ item2.type }}</cover-view
            >
          </block>
          <cover-view class="bottom" @click="getTrack">
            获取车辆轨迹
          </cover-view>
          ...
          <cover-image
            class="p-fixed"
            :class="[orderList.createTime ? 'after_image1' : 'order_image1']"
            :src="orderList.createTime ? beforeUrl1 : beforeUrl1"
          ></cover-image>
          <!-- 运输中 -->
          <cover-view
            class="order_status2 p-fixed"
            :style="[
          ...
          >
          <cover-view
            class="order_from2 p-fixed"
            :style="[
              { color: orderList.secondweighttime ? '#000' : '#b9b9b9' },
            ]"
            >已离开{{ orderList.factoryname }}，开往{{
          ...
          <cover-image
            ...
            :src="orderList.secondweighttime ? afterUrl2 : beforeUrl2"
          ></cover-image>
          <!-- 已完成 -->
          <template v-if="orderStatus === 0 ? !isActivity : isActivity">
            <cover-view class="order_status3 p-fixed">已完成</cover-view>
            <cover-view class="order_from3 p-fixed"
              >到达{{ orderList.customername }}</cover-view
            >
            <cover-image
              class="p-fixed"
              :class="[orderList.finishtime ? 'after_image3' : 'order_image3']"
              :src="orderList.finishtime ? afterUrl3 : beforeUrl3"
            ></cover-image>
...
export default {
  data () {
    return {
      leftList: [
        { name: '车牌号：', leftTop: 290 },
        { name: '客户名称：', leftTop: 310 },
        { name: '工厂名称：', leftTop: 330 },
        { name: '送达地区：', leftTop: 350 },
        { name: '物料名称：', leftTop: 370 },
        { name: '订单承重：', leftTop: 390 },
        { name: '开票时间：', leftTop: 410 },
        { name: '二次过磅时间：', leftTop: 430 },
        { name: '完成时间：', leftTop: 450 },
        { name: '完成方式：', leftTop: 470 },
        { name: '订单里程：', leftTop: 490 }
      ],
      polylines: [],
      rightList: [],
      latitude: '23.043594',
  ...
  onLoad () {
    ...
    uni.$on("test", (data) => {
      ...
      that.rightList = [
        { type: orderList.carno, rightTop: 290 },
        { type: orderList.customername, rightTop: 310 },
        { type: orderList.factoryname, rightTop: 330 },
        { type: orderList.areaname, rightTop: 350 },
        { type: orderList.goodsname, rightTop: 370 },
        { type: orderList.orderweight, rightTop: 390 },
        { type: orderList.createTime, rightTop: 410 },
        { type: orderList.secondweighttime, rightTop: 430 },
        { type: orderList.secondweighttime, rightTop: 450 },
        { type: orderList.finishtype, rightTop: 470 },
        { type: '30Km', rightTop: 490 }
      ]
